...
核心抽象为三类：**格式策略**（`PinyinFormat` 枚举）、**字典视图**（四张不可变映射常量）、**转换器**（`PinyinHelper` / `ChineseHelper` 两个无状态命名空间类型）。无并发设计、无 FFI、无 IO。
...
| `PinyinDicts` | 内部 `struct`/全局常量 | 字典视图聚合 |
...
├── pinyin_spec.mbt                  # 形式化契约（#declaration_only）
...
- **公开方法语义**：
  ...
  - `convert_to_traditional_chinese`：简→繁，**保留源库 O(n) 反查语义**（遍历 `CHINESE_MAP` 找值为输入字符的键），不主动优化为反向索引（需求 3.8 明确"不改变对外语义"，且源库此实现有歧义多映射行为，优化可能改变多映射场景的输出）。
...
- **数据结构选型**：
  ...
  - **为何 `Map` 而非 `HashMap`**：MoonBit 标准库 `Map` 是惯用不可变映射，字面量构造直接 `{ "k" => "v" }`，查询语义对等；源库 `HashMap` 是 Cangjie 的可变哈希表，移植版字典本身不可变（仅 `add_*_dict_resource` 需可变，见下）。
- **可变字典支持**：源库 `add_pinyin_dict_resource` 等通过 `HashMap.add(all:)` 原地合并。移植版需支持等价的追加自定义字典语义。设计为全局 `Ref[Map]` 或 `Mutex[Map]`（见 §六并发设计）。
...
- **四张基础字典**：构建期内嵌为字面量，运行时构造为不可变 `Map`，全局常量，无需同步。
- **自定义字典追加**：全局 `Ref[Map]` 或封装的可变字典视图。MoonBit 全局值默认不可变，可变全局需 `Ref` 或 `sync` 包原语。

### 6.2 自定义字典的并发安全策略

源库 `HashMap.add(all:)` 非线程安全，源库 `Reliability/` 压测也未验证并发追加。移植版设计：

- **默认实现**：全局 `Ref[Map]`，`add_*` 方法用 `dicts.pinyin_table.set(dict)` 替换为合并后的新 `Map`（`Map` 不可变，替换引用）。MoonBit `Ref` 赋值非原子，但单写多读场景下读侧可能看到旧值——这与源库语义一致（源库 `HashMap` 非线程安全，并发读可能看到部分更新）。
- **不引入 `Mutex`**：避免引入 `moonbitlang/async` 或 `sync` 依赖，保持纯计算库的零依赖特性；并发追加自定义字典是罕见场景，需求未要求线程安全保证。
- **保留语义对等**：单线程下行为与源库完全一致；多线程下源库本身无保证，移植版不主动提供更强保证。
...
- **为何不用 `HashMap`**：MoonBit 标准库无 `HashMap` 类型名（`Map` 即惯用映射）；源库 `HashMap` 是 Cangjie 命名，移植按目标语言惯例。
...
- 用 `declare` 关键字声明全部公开 API 签名（类型、方法、错误）。
...
| D5 | 字典数据结构 | `Map[Int, Int]` + `Map[String, String]` | MoonBit 惯用映射，字面量构造简洁 |
| D6 | 单字拼音字典内嵌 | 脚本生成 `.mbt` 字面量 | 最稳妥跨后端，编译期验证完整性 |
| D7 | 数据子包拆分 | 拆分 | 隔离生成代码，版本控制聚焦 |
| D8 | `convert_to_traditional_chinese` | 保留 O(n) 反查 | 需求 3.8 不主动优化，避免改变多映射语义 |
| D9 | 目标后端 | wasm/js/native 三后端 | 源库纯计算无 FFI，天然跨后端 |
| D10 | FFI | 无 | 源库无 C 依赖，不引入 c-binding skill |
| D11 | 自定义字典并发 | 全局 `Ref[Map]`，无 `Mutex` | 保留源库语义，避免引入 sync 依赖 |
...
| D14 | 测试策略 | spec 契约 + 分级黑盒 + snapshot | 对齐 `moonbit-spec-test-development` skill |
